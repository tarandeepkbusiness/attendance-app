// Updated SelectActivity with dynamic fetch
...
function SelectActivity() {
  ...
  // Fetch mandatory + elective activities from backend (action=activities)
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        ...
        // Expect data.activities to be an array of strings
        setActivities(data.activities || []);
      } catch (e) {
        console.error(e);
        // Fallback to hard‑coded list if needed
        setActivities([
          'Meditation & Sudh Gurbani',
  ...
  return (
    <div className="app-container animate-fade-in" style={{ padding: '1.5rem', paddingBottom: '80px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {activities.map(renderActivityButton)}
      </div>
